...
crime_lou <- crime_lou%>%
  filter(zip_code %in% zips & year >= 2007 & year <= 2016)
```
# Introduction
...
dataset. As usual, the complete code for this analysis can be found [here](https://github.com/Grollus/LouisvilleCrime/blob/master/Exploring_Time_Aspect_of_Thefts.Rmd).

It is my impression that police departments assume crime ebbs and flows based on various
...
by_dow <- crime_lou%>%
  ...
  summarise(count = n())%>%
  mutate(perc_below_max = round(((max(count)/count)-1)*100, 1))
...
midnight_peak <- round(((max(by_hour$count)/by_hour[by_hour$hour == 0,]$count) -1) *100, 1)
...
first_day_peak <- round(((max(by_day$count)/sort(by_day$count, partial = d-1)[d-1])-1) *100, 1)
last_day_valley <- -round(((min(by_day$count)/sort(by_day$count, partial = d-1)[d-1])-1) *100, 1)
```
...
a gradual falling off on either side. We see a `r by_month$perc_below_max[by_month$month == 'Feb']`%
...
but is not particularly insightful.  More interesting is the odd spike at 0 (midnight).
It could make sense that certain crimes are more prevalent during the late hours--for instance dui--
...
In fact, the midnight count is only `r midnight_peak`% off the max (right around 6pm).
...
```{r hdwm_full_dataset, echo = FALSE, cache = TRUE, out.width = '100%', warning = FALSE, message = FALSE}
library(grid)
bar.theme <- 
...
hourly <- crime_lou%>%
  ...
  geom_bar(stat = 'identity', fill = 'darkblue', alpha = .5, color = 'black', size = 0.28)+
  stat_smooth(method = 'auto', size = .25, alpha = .25, fill = "grey", colour = "red2")+
...
daily <- crime_lou%>%
  ...
  geom_bar(stat = 'identity', fill = 'darkblue', alpha = .5, color = 'black', size = .25)+
  stat_smooth(method = 'auto', size = .25, alpha = .25, fill = "grey", colour = "red2")+
...
dayOfWeek <- crime_lou%>%
  ...
  geom_bar(stat = 'identity', fill = 'darkblue', alpha = .5, color = 'black', size = .3)+
  stat_smooth(method = 'auto', size = .25, alpha = .25, fill = "grey", colour = "red2")+
...
monthly <- crime_lou%>%
  ...
  geom_bar(stat = 'identity', fill = 'darkblue', alpha = .5, color = 'black', size = .3)+
  stat_smooth(method = 'lm', size = .25, alpha = .25, fill = "grey", colour = "red2")+
...
THIS ANALYSIS SEEMS OUTDATED/WRONG. IF YOU PLOT THE PAST DECADE BY MONTH,
YOU SEE EXTREMELY CONSISTENT DATA ON EVERY DAY EXCEPT THE FIRST OF EACH MONTH.
I NEED TO ADJUST THIS SECTION TO TALK ABOUT THAT.
...
```{r, 2016_month_x_day, echo = FALSE, cache = TRUE,out.width = '100%', warning = FALSE, message = FALSE}
crime_lou%>%
  # filter(year == 2016)%>%
  group_by(month, day)%>%
  summarise(count = n())%>%
  ggplot(aes(x = day, y = count))+
  geom_bar(stat = 'identity', fill = 'darkblue', alpha = .5, color = 'black', size = .26)+
  facet_wrap(~month)+
  stat_smooth(method = 'auto', size = .25, alpha = .25, fill = 'grey', colour = 'red2')+
  scale_x_discrete(limits = seq(1, 30, by = 2))+
  labs(x = "Day", y = "")+
  ggtitle("Daily Crime Counts for 2016, by Month")+
  bar.theme+
  theme(axis.text.x = element_text(size = 5))+
  theme(strip.background = element_rect(fill = 'white', size = .25),
...
#   geom_bar(stat = 'identity', fill = 'darkblue', alpha = .5, color = 'black', size = .2)+
#   facet_wrap(~hour)+
...
# g1 <- ggplotGrob(p1)
# 
# pos1 <- c(subset(g1$layout, grepl('panel', g1$layout$name), select = t))
# for(i in pos1) g1$heights[i-1] = unit(0.3, 'cm')
# 
# grobs1 <- which(grepl('strip', g1$layout$name))
# for(i in grobs1) g1$grobs1[[i]]$heights <- unit(1, 'npc')
# grid::grid.newpage()
# grid::grid.draw(g1)
...
p2 <- crime_lou%>%
  ...
  summarise(count = n())%>%
  ggplot(aes(x = reporting, y = count))+
  geom_bar(stat = 'identity', fill = 'darkblue', alpha = .5, color = 'black', size = .20)+
  facet_wrap(~day)+
...
# Want to make the facet heading thinner so plots are a little more readable
g2<- ggplotGrob(p2)

pos2 <- c(subset(g2$layout, grepl('panel', g2$layout$name), select = t))
for(i in pos2) g2$heights[i-1] = unit(0.3, 'cm')

grobs <- which(grepl('strip', g2$layout$name))
for(i in grobs) g2$grobs[[i]]$heights <- unit(1, 'npc')
grid::grid.newpage()
grid::grid.draw(g2)
```
...
```{r hwm_ind_crimes, echo = FALSE, cache = TRUE, out.width = '100%', out.height = '100%', warning = FALSE, message = FALSE}
library(grid)
plot_x_by_hour <- function(df, variable, color){
  df%>%
    ...
    summarise_(count = ~n())%>%
    ggplot(aes(x = hour, y = count))+
    geom_bar(stat = 'identity', fill = color, alpha = .5, color = 'black', size = .25)+
    facet_grid(variable, scales = 'free_y')+
    stat_smooth(method = 'auto', size = .25, alpha = .3, fill = "grey", colour = "red2")+
...
observations, but the weights decay as they get older.  This means more recent 
...
Second, is an [Autoregressive Integrated Moving Average (ARIMA)](https://www.otexts.org/fpp/8) model. 
Unlike exponential smoothing, ARIMA models are looking at historical correlations of the variable of interest with itself
at different periods in the past(also known as lags). These autocorrelation terms are combined with moving average
...
for the three models fit with the [forecast](https://cran.r-project.org/web/packages/forecast/index.html) 
package. All these models are forecasting 6 months into the future.
...
usefulness of our models. The creation of the training and testing sets and the initial 
...
daily_train <- window(daily_ts, start = c(2007,1), end = 522.6)#the precise week number coinciding with end of 2015
...
# at a much more uncertain forecast, but that should not be surprising. Intuitively, we should
...
The ARIMA model is similar to the one used in the monthly model, but we include fourier covariate terms 
...
but a larger 'window' was used. This is because our seasonal pattern seems to stay 
constant through the years and a large window will tell the model to look at all the 
data while fitting.

Below are the forecast plots for the 3 models forecasting the first 4 months of 2016.
...
It should be noted that the best model for this data varies depending on the forecasting
...
we are dealing with, we can see hourly, weekly, monthly  and yearly seasonal effects.
...
of the time series. With the complexities inherent in the calendar year, it is quite likely
that seasonal periods ended up shifted by a day here or there.  This could have profound
...
One potential avenue is to continue to explore the potential benefits of constructing dummy
variable regressors. Adding relevant data--such as the occurance of a holiday or 
the occurance of any number of weather related events--to the model will only help since we
are currently ignoring known, important information. 

Another area to explore is the inclusion of spatial data into the
