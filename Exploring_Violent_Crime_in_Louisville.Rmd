---
title: "Is Violent Crime in Louisville on the Rise?"
...
of the apocalypse. This is the first in, hopefully, a series of posts in which I will 
dive into the data and investigate just how bad the crime has gotten in our fair city.
...
2005-2015.  Given the breadth of the available data, I will need to split the analysis
...
```{r, base_filtering, echo = FALSE, cache = TRUE, warning = FALSE, message = FALSE}
crime_lou <- crime_lou%>%
  filter(year <=2015 & year >=2005 & !is.na(lat)  & !is.na(lng))
```

# Overview of Violent Crime
2015 was a year with a record number of homicides in Louisville. Seeing these reports every day 
...
The black trend line fit to the data has a slope of about 10.4 meaning (with this enormously
oversimplified model) that we are seeing an increase of about 10.4 violent crime reports
every year.
...
11 year mean, but it is down slightly from the peak level which occured in 2007.
...
So it does seem like we are seeing a trend up in violent crime.  But looking at the graph
it sure doesn't seem like the end of the world scenario the news is predicting.  Was 2015
even a significant increase from the general trend of the past decade?  This is a more complicated
question than it appears.  Are we asking about 2015 compared to the running average 
from the previous 10 years(2005-2014)? Or are we asking whether 2015 counts are different from
2014(or different from 2005 for that matter)? Let's look at both and see what they tell us.
...
violent crime. However, when we compare 2015 versus 2005-2014, the distribution does appear
to have shifted right, indicating a higher average daily crime count.

```{r, daily_count_by_year, echo = FALSE, cache = TRUE, out.width = '1100px', warning = FALSE, message = FALSE}
...
ggplot(daily_sums_by_year%>%filter(year != 2015), aes(x = count, fill = 'darkblue'))+
  geom_density(alpha = .4)+
  geom_density(data = daily_sums_by_year%>%filter(year == 2015), aes(x = count, fill = 'darkred'), 
               alpha = .4)+
...
With all this in hand, can we give a more quantative answer as to whether violent crime
has increased? We can if we ask the question 'Is the mean count of violent crimes
per day in 2015 different from the mean count of violent crimes in the period 2005-2015'?
...
10.3, how likely is it that we see a random sample(the data from 2015) with a mean of 10.99.
The p-value of .0007 is basically the probability of seeing this result. So in this case,
the result is pretty unlikely if it were just due to chance(i.e., there is a real difference).
Looking at the confidence interval, if our 2005-2015 mean of 10.3 was contained within
it would indicate that we could not be confident 2015's results weren't due to random 
fluctuation.  Both of these values indicate that the mean of 2015 is different, and in
this case larger than, 2005-2014.
...
`r round(mean(sums_2015$count) -mean(sums_2005_2014$count), 2)`.  Is .69 more violent
...
Louisville's population grew 2.6% from 2010 to 2014.  It seems logical that
an increase in population will also result in an increase in crime numbers. If you normalize for population
...
it is simple enough to see where violent crime is happening.
...
ggmap(louisvilleMap)+
  ...
  ggtitle("Louisville Violent Crime, 2005-2015")+
  density.theme+
...
hot_spots_map <- get_map(location = "40203", source = 'google',
...
ggmap(hot_spots_map)+
  stat_bin2d(
    aes(x = lng, y = lat),
    data = violent_crime,
    size = .00001, bins = 30, alpha = .8, color = "gray20"
  )+
  facet_wrap(~year)+
  scale_fill_viridis(option = "inferno", name = "Incident Count")+
  ggtitle("Louisville Violent Crime, 2005-2015")+
...
When we zoom in and break it down by year, we do see a minor, but visible, increase
in the intensity of the heat map.  Particularly from 2012-2015 we see a significant 
brightening of intensity in downtown Louisville. What becomes apparent when viewing 
these breakdowns is just how localized the violent crime is.  When broken down by zip
codes, it is shocking how skewed the numbers are.  Just 6 of Louisville's 38 zip codes
...
Interestingly, only one (two if we are being generous and considering 40212's increase
to be sufficiently large) of those zip codes has seen a stark increase in violent crime. 40211's 
increase is so large that merely by excluding it from our analysis we see drastically 
different results.  
...
p5 <- crime_lou%>%
  ...
  summarise(count = n())%>%
    ...
    stat_smooth(method = 'lm', size = 1.25, alpha = .8, fill = "grey", colour = "black")+
    geom_bar(stat = 'identity', colour = "black",fill = "darkblue",  alpha = .5)+
    ggtitle("Violent Crime Without 40211")+
    labs(x = "Year", y = "Incident Count")+
    scale_x_discrete(limits = seq(2005, 2015, by = 1))+
    bar.theme
...
daily_sums_by_year_no_40211 <- crime_lou%>%
  filter((nibrs_code == '13a' |nibrs_code == '120'| nibrs_code == '09a'|nibrs_code == '11a')&
           zip_code != 40211)%>%
  group_by(year = as.factor(year), yday = as.factor(yday))%>%
  summarise(count = n())%>%
...
sums_2015_no_40211 <- daily_sums_by_year_no_40211%>%
  filter(year == 2015)

sums_2005_2014_no_40211 <- daily_sums_by_year_no_40211%>%
  filter(year != 2015)
...
t <- t.test(sums_2015_no_40211$count, mu = mean(sums_2005_2014_no_40211$count))
```

Notice now that the trend line actually is negative when 40211 is excluded.  If we 
perform the same t-test that we did before, we no longer have a significant finding.
Our p-value is no longer significant and the mean number of violent crimes per day from 
2005-2014 is contained within the confidence interval of the mean value for 2015.
Both of these indicate that we can't say with any certainty that the average daily 
crime counts are different.
...
# Conclusion
As with most socioeconomic matters, violent crime in Louisville is tough to pin down.
On the whole, we did seem to have a slightly above average 2015 when compared to the
previous 10 years.  However, whether this marginal trend is cause for alarm is a different 
matter. The heavily localized nature of violent crime in the city seems to suggest
...
Louisville may have actually seen a very minor decrease in violent crime over the past decade.

On the other hand, city officials may have great reason to be concerned as 
several regions of the city have experienced rapid increases in violent crime
over the past decade. Regions downtown account for a vast majority of all violent
crime in the city and certainly have devastating economic and social impacts on, not
only those specific regions, but greater downtown Louisville as a whole. This is, obviously,
...
It should be noted that this is my first dive in crime data.  Therefore, many
...
The full code used to generate this report can be found [here](www.github.com).