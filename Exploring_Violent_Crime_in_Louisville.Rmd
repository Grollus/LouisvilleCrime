...
The red trend line fit to the data has a slope of about 10.4 meaning (with this enormously
...
it sure doesn't appear to be the end of the world scenario the news is predicting.  Was 2015
...
ggplot(daily_sums_by_year, aes(x = count, color = year))+
  ...
  density.theme
```
However, when we compare 2015 versus 2005-2014, a clear shift in the distribution
is visible.  
...
per day in 2015 different from the mean count of violent crimes in the period 2005-2014'?
...
Looking at the confidence interval, if our 2005-2014 mean of 10.3 was contained within
it would indicate that we could not be confident 2015's results weren't due to random 
fluctuation.  Both of these values indicate that the mean of 2015 is different from, and in
this case larger than, the 2005-2014 mean.
...
in the intensity of the heat map.  Particularly from 2012-2015 we see a brightening
of intensity in downtown Louisville. What becomes apparent when viewing 
...
p5 <- crime_lou%>%
  ...
  summarise(count = n())%>%
    ...
    stat_smooth(method = 'lm', size = 1, alpha = .3, fill = "grey", colour = "red2")+
    geom_bar(stat = 'identity', colour = "black",fill = "darkblue",  alpha = .5)+
    ggtitle("Violent Crime Excluding 40211")+
    labs(x = "Year", y = "Incident Count")+
    scale_x_discrete(limits = seq(2005, 2015, by = 1))+
    bar.theme
...
tough to pin down. On the whole, we did seem to have a slightly above average 2015 when compared to the
...
On the other hand, city officials may have great cause for concerned as 
...
crime in the city and that certainly has a devastating economic and social impact on, not
